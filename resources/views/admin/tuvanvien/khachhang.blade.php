@extends('master')

@section('css')
    <link href="{{ asset('assets/vendor/datatables/dataTables.bootstrap4.min.css') }}" rel="stylesheet">
@endsection

@section('content')
    <div class="container-fluid">
        <div class="card shadow mb-4">
            <div class="card-header py-3" style="display: flex; align-items: center;">
                <h6 class="m-0 font-weight-bold text-primary" style="flex: 1;">Khách hàng của tư vấn viên @if ($tvv_nguoidung && $tvv_nguoidung->count() > 0)
                        {{ $tvv_nguoidung->first()->tenTvv }}
                    @endif
                </h6>

                <a href="javascript:history.back()" class="m-0 font-weight-bold text-primary" style="float: right;">Back</a>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <div id="accList">
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    <th>STT</th>
                                    <th>Tên khách hàng</th>
                                    <th>Số điện thoại</th>
                                    <th>Số lượng que thai</th>
                                    <th>Số lượng que trứng</th>
                                    <th>Biểu đồ nồng độ LH</th>
                                    <th>Thời gian</th>
                                </tr>
                            </thead>
                            <tfoot>
                                <tr>
                                    <th>STT</th>
                                    <th>Tên khách hàng</th>
                                    <th>Số điện thoại</th>
                                    <th>Số lượng que thai</th>
                                    <th>Số lượng que trứng</th>
                                    <th>Biểu đồ nồng độ LH</th>
                                    <th>Thời gian</th>
                                </tr>
                            </tfoot>
                            <tbody>
                                @foreach ($tvv_nguoidung as $item)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $item->tenNguoiDung }}</td>
                                        <td>{{ $item->taiKhoan }}</td>
                                        <td>{{ $item->soLuongQueThai }}</td>
                                        <td>{{ $item->soLuongQueTrung }}</td>
                                        <td>
                                            <a href="{{ URL::to('/admin/chart/details-user/' . $item->maNguoiDung) }}"
                                                 class="btn btn-sm btn-primary information"
                                                data-id="" data-toggle="tooltip" title="">
                                                <i class="far fa-heart"></i>
                                            </a>
                                        </td>
                                        <td>{{ $item->thoiGian }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>

    </div>
@endsection

@section('scripts')
    <!-- Bootstrap core JavaScript-->
    <script src="{{ asset('assets/vendor/jquery/jquery.min.js') }}"></script>
    <script src="{{ asset('assets/vendor/bootstrap/js/bootstrap.bundle.min.js') }}"></script>

    <!-- Core plugin JavaScript-->
    <script src="{{ asset('assets/vendor/jquery-easing/jquery.easing.min.js') }}"></script>

    <!-- Custom scripts for all pages-->
    <script src="{{ asset('assets/js/sb-admin-2.min.js') }}"></script>

    <!-- Page level plugins -->
    <script src="{{ asset('assets/vendor/datatables/jquery.dataTables.min.js') }}"></script>
    <script src="{{ asset('assets/vendor/datatables/dataTables.bootstrap4.min.js') }}"></script>

    <!-- Page level custom scripts -->
    <script src="{{ asset('assets/js/demo/datatables-demo.js') }}"></script>
@endsection
